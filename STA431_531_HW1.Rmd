...
tennis_path <- "C:/Users/Katie/Downloads/tennis.txt"
# If file is not found, skip execution gracefully
if (file.exists(tennis_path)) {
  tennis.dfr <- read_tennis_data(tennis_path)
  cat("Successfully loaded tennis data with", nrow(tennis.dfr), "players\n")
} else {
  tennis.dfr <- NULL
  cat("tennis.txt not found at:", tennis_path, "\n")
}
```

Add winning percentage columns:

```{r q2-add-cols, echo=TRUE}
if (is.null(tennis.dfr)) {
  cat("Skipping tennis data operations - tennis.dfr is NULL\n")
} else {
with(tennis.dfr, {
...
if (!is.null(tennis.dfr)) {
  ...
  cat("Skipping sort - tennis.dfr is NULL\n")
}
```
...
if (!is.null(tennis.dfr)) {
  ...
  cat("Skipping nested sort - tennis.dfr is NULL\n")
}
```
...
if (!is.null(tennis.dfr)) {
  ...
  cat("Skipping subset - tennis.dfr is NULL\n")
}
```
...
if (!is.null(tennis.dfr)) {
  ...
  cat("Skipping means calculation - tennis.dfr is NULL\n")
}
```
...
```{r q3-write}
if (!is.null(tennis.dfr)) {
  output_path <- "C:/Users/Katie/Downloads/tennis_out.txt"
  write.table(
    tennis.dfr,
    file = output_path,
    quote = FALSE,        
  ...
  cat("Skipping file write - tennis.dfr is NULL\n")
}
```
