...
import { CHILD_MARRIAGE_RING_KEYS } from '@/data/childrenPlanner'
import {
...
export default function ChildrenPlannerTab() {
  ...
  return (
    <div className="space-y-6">
      <div className="text-sm text-gray-400 leading-relaxed rounded-lg border border-gray-700 bg-gray-900/40 px-4 py-3">
        Ring counts are stored on your <strong className="text-gray-200">inventory</strong> under{' '}
        <code className="text-amber-200/90">BlackPearlRing3</code>, <code className="text-amber-200/90">RubyRing2</code>,{' '}
        <code className="text-amber-200/90">AmethystRing1</code> (legacy lowercase keys are read too). Enter each
        child&apos;s <strong>total attributes</strong>; we rank them, then pick the top N per ring pool where N is rings
        owned. <strong>Marriage gain</strong> below is the sum of those selected children&apos;s totals again: their own
        power is already in your DOM today; when they marry, a spouse from the ring match is modeled as contributing
        about the same amount once more — so the number is the extra DOM you expect (e.g. three children 100K+60K+55K ⇒
        +215K), not merged into the main calculator bar.
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {ringCards.map((r) => (
          <Card key={r.key} className="bg-gray-800/50 border-gray-600">
                  ...
                  {childrenPlannerEntries.map((row) => {
                    ...
                    return (
                      <tr key={row.id} className="border-t border-gray-800">
                          ...
                          {tier ? (
                            <span>
                              ...
                              <span className="text-gray-500">
                                ({tier.ring === 'blackPearl' ? 'Black pearl' : tier.ring === 'ruby' ? 'Ruby' : 'Amethyst'})
                              </span>
                            </span>
                          ) : (
                        ...
                        <td className="p-2">
                          <Button type="button" variant="ghost" size="sm" className="text-red-400" onClick={() => removeEntry(row.id)}>
                            Remove
                          </Button>
                        </td>
                      </tr>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="bg-gray-800/50 border-gray-600">
          ...
          <CardContent className="space-y-4 text-sm">
            <div>
              <div className="text-gray-400 text-xs mb-1">Black pearl ({selection.selectedBlackPearl.length} / {ringCounts.blackPearl})</div>
            ...
            <div>
              <div className="text-gray-400 text-xs mb-1">Ruby ({selection.selectedRuby.length} / {ringCounts.ruby})</div>
              <ul className="list-disc pl-5 text-gray-200 space-y-1">
            ...
            <div>
              <div className="text-gray-400 text-xs mb-1">Amethyst ({selection.selectedAmethyst.length} / {ringCounts.amethyst})</div>
              <ul className="list-disc pl-5 text-gray-200 space-y-1">
        ...
        <Card className="bg-gray-800/50 border-green-900/40">
          ...
          <CardContent className="space-y-2">
            ...
            <p className="text-xs text-gray-400 leading-relaxed">
              Same as the sum of the selected children&apos;s totals: each child&apos;s value is already in your current
              DOM; this line is the approximate <strong className="text-gray-300">spouse-side</strong> contribution when
              each marries someone of similar strength (e.g. 100K child + ~100K spouse ⇒ +100K net vs today for that
              slot). Not added to the main calculator totals.
            </p>
          </CardContent>
        </Card>
      </div>
